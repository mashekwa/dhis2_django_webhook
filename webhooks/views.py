...
from .tasks import get_event_data, get_hmis_code, process_webhook_data
...
def webhook_receiver(request):
    ...
    try:
        ...
        json_data = json.loads(raw_data)  # Parse JSON data
        print(json_data)
    except json.JSONDecodeError:
        return JsonResponse({'status': 'error', 'message': 'Invalid JSON'}, status=400)

    # Ensure the required field 'HhNhMHtKYiB' exists
    if not json_data.get('HhNhMHtKYiB'):
        return JsonResponse({'status': 'success', 'message': 'No LAB CODE'}, status=500)

    # Offload processing to a Celery task
    process_webhook_data.delay(json_data)  # Run task asynchronously
    return JsonResponse({'status': 'success', 'message': 'Processing started'}, status=202)
